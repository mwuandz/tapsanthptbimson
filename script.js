...
function render(dir='next'){
  ...
  if(mobile){left.innerHTML='';right.innerHTML=pageHTML(current);}
  else{const start=current%2===0?current:current-1;left.innerHTML=pageHTML(start);right.innerHTML=pageHTML(start+1);current=start;}
  ...
  $('#pageLabel').textContent=mobile?`Trang ${current+1} / ${pages.length}`:`Trang ${current+1}${current+2<=pages.length?'–'+(current+2):''} / ${pages.length}`;
  $('#pageRange').value=current+1;$('#chapterTitle').textContent=pages[current].title;
  $('#prevBtn').disabled=current===0;$('#nextBtn').disabled=mobile?current===pages.length-1:current>=pages.length-2;
  document.querySelectorAll('.thumb').forEach((el,i)=>el.classList.toggle('active',i===current||( !mobile && i===current+1)));
  bindZoom();
}
function go(delta){const step=isMobile()?1:2;const n=Math.max(0,Math.min(pages.length-1,current+delta*step));if(n===current)return;current=n;playFlip();render(delta>0?'next':'prev')}
...
$('#openBookBtn').onclick=()=>{landing.hidden=true;reader.hidden=false;render();window.scrollTo({top:0,behavior:'smooth'})};
$('#closeReaderBtn').onclick=()=>{reader.hidden=true;landing.hidden=false};
