...
function render(dir='next'){
  ...
  $('#pageLabel').textContent=coverMode?`Bìa · Trang 1 / ${pages.length}`:`Trang ${current+1}${current+2<=pages.length?'–'+(current+2):''} / ${pages.length}`;
  ...
  document.querySelectorAll('.thumb').forEach((el,i)=>el.classList.toggle('active',i===current||(!coverMode&&i===current+1)));
  bindZoom();
}
function go(delta){const step=(current===0||delta<0&&current===1)?1:2;const n=Math.max(0,Math.min(pages.length-1,current+delta*step));if(n===current)return;current=n;playFlip();render(delta>0?'next':'prev')}
